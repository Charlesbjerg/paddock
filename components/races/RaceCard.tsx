...
export default function RaceCard({
	...
	circuit: string;
	circuitFlag: string;
	results: RaceMiniResults;
}) {
	return (
		<Link
			href={`/races/${race.raceId}`}
			className="px-4 py-6 bg-white text-black rounded-lg shadow-lg shadow-transparent transition-all hover:shadow-white/30 hover:opacity-90 hover:-translate-y-1"
		>
			...
			<p className="mb-4">
				{race.date && formatRaceDate(race.date)} at{" "}
				{race.time && formatRaceStartTime(race.time)}
			</p>
			<div className="flex flex-wrap items-bottom gap-x-4 gap-y-4">
				...
				<FastestLap
					driver={results.fastest.driver}
					time={results.fastest.time}
				/>
			</div>
		</Link>
	);
